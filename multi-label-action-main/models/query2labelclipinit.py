...
class Query2LabelCLIPInit(nn.Module):
...
class Query2LabelCLIPInitExecutor:
    def __init__(self, train_loader, test_loader, criterion, eval_metric, class_list, test_every, distributed, gpu_id) -> None:
        ...
        q2linit = Query2LabelCLIPInit(class_embed).to(gpu_id)
        if distributed: 
            self.q2linit = DDP(q2linit, device_ids=[gpu_id], find_unused_parameters=False)
        else: 
            self.q2linit = q2linit
        for p in self.q2linit.parameters():
            p.requires_grad = True
        self.optimizer = Adam([{"params": self.q2linit.parameters(), "lr": 0.00001}])
    ...
    def save(self, file_path="./checkpoint.pth"):
        backbone_state_dict = self.q2linit.backbone.state_dict()
        transformer_state_dict = self.q2linit.transformer.state_dict()
        query_embed_state_dict = self.q2linit.query_embed.state_dict()
        fc_state_dict = self.q2linit.fc.state_dict()
    ...
    def load(self, file_path):
        ...
        self.q2linit.backbone.load_state_dict(checkpoint["backbone"])
        self.q2linit.transformer.load_state_dict(checkpoint["transformer"])
        self.q2linit.query_embed.load_state_dict(checkpoint["query_embed"])
        self.q2linit.fc.load_state_dict(checkpoint["fc"])
