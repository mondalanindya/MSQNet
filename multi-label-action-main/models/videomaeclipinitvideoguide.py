...
from transformers import VideoMAEModel, CLIPTokenizer, CLIPTextModel, CLIPVisionModel, logging


class VideoMAECLIPInitVideoGuide(nn.Module):
...
class VideoMAECLIPInitVideoGuideExecutor:
    def __init__(self, train_loader, test_loader, criterion, eval_metric, class_list, test_every, distributed, gpu_id) -> None:
        ...
        model = VideoMAECLIPInitVideoGuide(class_embed, num_frames).to(gpu_id)
        if distributed: 
